    ...
    public function addTransfer($data = array(), $items = array(),$id=null)
    {
        ...
        if ($this->db->insert('transfers', $data)) {
            ...
            foreach ($items as $item) {
                ...
                if($id) $this->db->update('purchase_items', array('tr_remain_qty'=>$item['tr_remain_qty']), array('purchase_id' => $id,'product_id' => $item['product_id']));
                if ($status == 'sent' || $status == 'completed') {
                    $this->syncTransderdItem($item['product_id'], $data['from_warehouse_id'], $item['quantity'], $item['option_id'],$item['product_id']);
                }
            }
        ...
        if ($ostatus == 'sent' || $ostatus == 'completed') {
            foreach ($oitems as $item) {
                ...
                if ($delete) {
                    ...
                    if ($ppi = $this->site->getPurchasedItem($clause)) {
                        $quantity_balance = $ppi->quantity_balance + ($item->quantity_balance - $item->quantity);
                        $this->db->update('purchase_items', array('quantity_balance' => $quantity_balance), array('id' => $ppi->id));
                    } else {
                        $clause['quantity'] = $item->quantity;
                        $clause['item_tax'] = 0;
                        $clause['quantity_balance'] = ($item->quantity_balance - $item->quantity);
                        $clause['status'] = 'received';
                        $this->db->insert('purchase_items', $clause);
                    }
                }
...
//            ->where('purchases.warehouse_id',$purchase_id)->where('purchases.status','received');
    ...
    {
        $this->db->select('products.id, code, name, warehouses_products.quantity, cost, tax_rate, type, unit, purchase_unit, tax_method,purchase_items.tr_remain_qty  as prqty')
            ->join('warehouses_products', 'warehouses_products.product_id=products.id', 'left')
