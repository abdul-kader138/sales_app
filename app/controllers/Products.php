...
class Products extends MY_Controller {
    ...
    function getProducts($warehouse_id = NULL) {
        ...
        $detail_link2 = anchor('products/PurchaseHistory/$1', '<i class="fa fa-file-text-o"></i> ' . lang('Purchase History'), 'data-toggle="modal" data-target="#myModal"');
        //$producthistory = '<center><button type="button" class="view_h btn btn-default btn-xs btn-primary" data-toggle="modal" data-target="#myModal">'.lang('View').'</button></center>'; 
        ...
        if ($warehouse_id) {
            $this->datatables
                    ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(wp.quantity, 0) as quantity, wp.rack as rack,'','',alert_quantity,(SELECT COUNT(p.id) FROM sma_purchases p JOIN sma_purchase_items pi ON pi.purchase_id = p.id where pi.product_id = sma_products.id) as count", FALSE)
                    ->from('products');
            ...
            //$this->datatables->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, {$this->db->dbprefix('products')}.cost as cost, {$this->db->dbprefix('products')}.price as price, COALESCE({$this->db->dbprefix('products')}.quantity, 0) as quantity, sub_categories.name as subcat, '' as rack, {$this->db->dbprefix('products')}.alert_quantity, COUNT({$this->db->dbprefix('purchases')}.id) as count", FALSE)
            $this->datatables->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, {$this->db->dbprefix('products')}.cost as cost, {$this->db->dbprefix('products')}.price as price, COALESCE({$this->db->dbprefix('products')}.quantity, 0) as quantity, sub_categories.name as subcat, '' as rack, {$this->db->dbprefix('products')}.alert_quantity, (SELECT COUNT(p.id) FROM sma_purchases p JOIN sma_purchase_items pi ON pi.purchase_id = p.id where pi.product_id = sma_products.id) as count", FALSE)
                    ->from('products')
                    ->join('categories', 'products.category_id=categories.id', 'left')
                    ->join('units', 'products.unit=units.id', 'left')
                    ->join('brands', 'products.brand=brands.id', 'left')
                    ->join('categories as sub_categories', 'products.subcategory_id=sub_categories.id', 'left')
                    ->group_by("products.id");
...
//            exit;
        ...
        //$this->datatables->add_column("Purchase History", $producthistory);
    ...
    function PurchaseHistory($id = NULL) {
        $data['id'] = $id;
        $this->load->view($this->theme . 'products/purchase_history', $data);
    }
    ...
    function add($id = NULL) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'cost' => $this->sma->formatDecimal($this->input->post('cost')) ? $this->sma->formatDecimal($this->input->post('cost')) : NULL,
                'price' => $this->sma->formatDecimal($this->input->post('price')) ? $this->sma->formatDecimal($this->input->post('price')) : NULL,
        ...
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos)) {
            ...
            $this->data['code_number'] = $this->site->getCodeProductPrefix();
    ...
    function details_search() {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('product_details_search')));
        $meta = array('page_title' => lang('product_details_search'), 'bc' => $bc);
        $this->page_construct('products/product_details', $meta, $this->data);
    }


    function getProductDetailsReport($pdf = NULL, $xls = NULL) {

        $this->sma->checkPermissions('details_search');
    ...
    function reorder_details() {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('product_reorder_report')));
        $meta = array('page_title' => lang('product_reorder_report'), 'bc' => $bc);
        $this->page_construct('products/product_reorder', $meta, $this->data);
    }


    function getProductReorderReport($pdf = NULL, $xls = NULL) {

        $this->sma->checkPermissions('reorder_details');
        ...
        if ($pdf || $xls) {
            ...
            if($warehouse){
                ...
                {$this->db->dbprefix('products')}.id as id", FALSE)
                    ...
                    ->join('warehouses_products', 'products.id=warehouses_products.product_id', 'left')
                    ->where('warehouses_products.warehouse_id', $warehouse)
                    ->where('warehouses_products.quantity <= products.alert_quantity or (warehouses_products.quantity is Null and products.alert_quantity is not Null)')
                    ->group_by('products.code');
                ...
                {$this->db->dbprefix('products')}.id as id", FALSE)
                    ...
                    ->join('units', 'products.unit=units.id', 'left')
                    ->join('brands', 'products.brand=brands.id', 'left')
                    ->where('products.quantity <= products.alert_quantity or (products.quantity is Null and products.alert_quantity is not Null)')
                    ->group_by('products.code');
            }
