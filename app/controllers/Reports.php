...
class Reports extends MY_Controller {
    ...
    function sales_margin() {
        ...
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('sales_report')));
        $meta = array('page_title' => lang('sales_margin_report'), 'bc' => $bc);
        $this->page_construct('reports/sales_margin', $meta, $this->data);
    }
    ...
    function getSalesMarginReport($pdf = NULL, $xls = NULL) {
        $this->sma->checkPermissions('sales', TRUE);
        $product = $this->input->get('product') ? $this->input->get('product') : NULL;
        $user = $this->input->get('user') ? $this->input->get('user') : NULL;
        $customer = $this->input->get('customer') ? $this->input->get('customer') : NULL;
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("date, reference_no, biller, customer, CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, ' (', " . $this->db->dbprefix('sale_items') . ".quantity, ')') as iname, sum( {$this->db->dbprefix('sale_items')}.quantity *  {$this->db->dbprefix('products')}.cost) as actual_price,sum( {$this->db->dbprefix('sale_items')}.quantity *  {$this->db->dbprefix('sale_items')}.net_unit_price) as sl_price, payment_status", FALSE)
                ->from('sales')
                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
                ->join('products', 'products.id=sale_items.product_id', 'left')
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
                ->group_by('sale_items.sale_id,sale_items.product_id')
                ->order_by('sale_items.sale_id asc');
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->customer);
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->iname);
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->actual_price);
                    $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->sl_price);
                    $this->excel->getActiveSheet()->SetCellValue('H' . $row, ($data_row->sl_price - $data_row->actual_price));
                    $this->excel->getActiveSheet()->SetCellValue('I' . $row, lang($data_row->payment_status));
                    $total += $data_row->actual_price;
                    $paid += $data_row->sl_price;
                    $balance += ($data_row->sl_price - $data_row->actual_price);
                    $row++;
                }
                $this->excel->getActiveSheet()->getStyle("F" . $row . ":H" . $row)->getBorders()
                    ->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, biller, customer, FSI.item_nane as iname, FSI.actual_price, FSI.sl_price, ((ROUND(FSI.sl_price,2))-(ROUND(FSI.actual_price,2))) as balance, payment_status, {$this->db->dbprefix('sales')}.id as id", FALSE)
                ->from('sales')
                ->join($si, 'FSI.sale_id=sales.id', 'left')
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left');
            // ->group_by('sales.id');
    ...
    public function time_schedule($id = 0, $template = '') {
        $this->sma->checkPermissions();
        ...
        switch ($template) {
            case 'export':
                $this->db->select('id, first_name, last_name, company');
                $this->data['ts_users'] = $this->db->get('users')->result();
                $this->data['type'] = $this->input->get('export_type');
                $this->load->view($this->theme . 'reports/time_schedule_export', $this->data);
                break;

            case 'edit':
                $this->data['count_time_sched'] = 0;
                $this->data['multi_time_sched'] = array();

                if ($id) {
                    $objTimeSchedule = $this->db->select('time_schedule.time_date,time_schedule.time_in,time_schedule.time_out, time_schedule.user_id,time_schedule_status.handle')
                                    ->from('time_schedule')
                                    ->join('time_schedule_status', 'time_schedule_status.time_schedule_status_id = time_schedule.time_schedule_status_id', 'left')
                                    ->where('time_schedule_id', $id)
                                    ->get()->row();

                    $this->data['obj_time_sched'] = $objTimeSchedule;

                    if ($objTimeSchedule) {
                        $this->db->select('time_schedule_id AS timeSheduleID, time_in AS timeIn, time_out AS timeOut');
                        $this->db->where('user_id', $objTimeSchedule->user_id);
                        $this->db->where('time_date', $objTimeSchedule->time_date);
                        $objTimeScheduleSet = $this->db->get('time_schedule')->result();

                        $this->data['count_time_sched'] = count($objTimeScheduleSet);
                        $this->data['multi_time_sched'] = $objTimeScheduleSet;
                    }
                }
                ...
                $this->db->select('id, first_name, last_name, company');
                $this->data['ts_users'] = $this->db->where('id', $objTimeSchedule->user_id)->get('users')->result();

                $this->load->view($this->theme . 'reports/time_schedule_edit', $this->data);
                break;

            default:
                $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('Time Schedule Report')));
                $this->data['ts_users'] = $this->db->get('users')->result();
                $meta = array('page_title' => lang('Time Schedule Report'), 'bc' => $bc);
                $this->page_construct('reports/time_schedule', $meta, $this->data);
        }
    }
    ...
    public function get_time_schedule() {
        ...
        $action .= "<button type=\"button\" class=\"btn btn-default btn-xs btn-primary dropdown-toggle\" data-toggle=\"dropdown\">";
        $action .= lang('actions') . " <span class=\"caret\"></span></button>";
        $action .= "<ul class=\"dropdown-menu pull-right\" role=\"menu\">";
        $action .= "<li>";
        $action .= "<a class=\"tip\" title=\"" . lang("view_report") . ' / ' . lang("edit") . "\" onclick=\"window.TimeSchedulerClass.populate($1)\" href=\"" . site_url('reports/time_schedule/$1/edit') . "\" data-toggle=\"modal\" data-target=\"#myModal\"><i class=\"fa fa-edit\"></i>" . lang("view_report") . ' / ' . lang("edit") . "</a>";
        $action .= "</li>";
        $action .= "</ul>";
        $action .= "</div></div>";
        ...
        $objDatatables = $this->datatables->select("time_schedule_id, sma_users.id, sma_users.first_name, sma_users.last_name, time_date, MIN(time_in) AS time_in, MAX(time_out) AS time_out, TIME(SUM(TIMEDIFF(time_out, time_in))) AS total_time, sma_time_schedule.ip_address, sma_time_schedule_status.name, sma_time_schedule_status.handle", FALSE)
                ->from("sma_time_schedule")
                ->join('sma_users', 'sma_users.id = sma_time_schedule.user_id')
                ->join('sma_time_schedule_status', 'sma_time_schedule.time_schedule_status_id = sma_time_schedule_status.time_schedule_status_id')
                ->group_by('time_date, sma_users.id')
                ->add_column("Actions", $action, "time_schedule_id");
        
        $strFirstName = trim($this->input->post('sSearch_2'));
        $strLastName = trim($this->input->post('sSearch_3'));
        $strDate = trim($this->input->post('sSearch_4'));
        $strStatus = trim($this->input->post('sSearch_9'));

        if (!empty($strFirstName)) {
            $objDatatables->like('sma_users.first_name', $strFirstName);
        }
        if (!empty($strLastName)) {
            $objDatatables->like('sma_users.last_name', $strLastName);
        }
        if ($strDate && $strDate = date('Y-m-d', strtotime($strDate))) {
            $objDatatables->where('time_date', $strDate);
        }
        if (!empty($strStatus)) {
            $objDatatables->like('sma_time_schedule_status.name', $strStatus);
        }

        if ($this->input->post('userId') && $this->input->post('userId') != '') {
            $objDatatables->where('sma_users.id', $this->input->post('userId'));
        }

        if ($this->input->post('fromDateTime') && $this->input->post('fromDateTime') != '') {
            $fromDateTime = explode(' ', $this->input->post('fromDateTime'));
            $fromDate = DateTime::createFromFormat('d/m/Y', $fromDateTime[0])->format('Y-m-d');
            $fromTime = $fromDateTime[1];
            if ($this->input->post('toDateTime') && $this->input->post('toDateTime') != '') {
                $toDateTime = explode(' ', $this->input->post('toDateTime'));
                $toDate = DateTime::createFromFormat('d/m/Y', $toDateTime[0])->format('Y-m-d');
                $toTime = $toDateTime[1];
                $objDatatables->where('time_date > "' . $fromDate . '" AND time_date < "' . $toDate . '"');
                $objDatatables->or_where('time_date = "' . $fromDate . '" AND time_in >= "' . $fromTime . '"');
                $objDatatables->or_where('time_date = "' . $toDate . '" AND time_out <= "' . $toTime . '"');
            } else {
                $objDatatables->where('time_date >=', $fromDate);
                $objDatatables->where('time_in >=', $fromTime);
            }
        }
        ...
        if ($this->input->post('toDateTime') && $this->input->post('toDateTime') != '') {
            ...
            if ($this->input->post('fromDateTime') && $this->input->post('fromDateTime') != '') {
                ...
                $fromDate = DateTime::createFromFormat('d/m/Y', $fromDateTime[0])->format('Y-m-d');
                $fromTime = $fromDateTime[1];
                $objDatatables->where('time_date < "' . $toDate . '" AND time_date > "' . $fromDate . '"');
                $objDatatables->or_where('time_date = "' . $toDate . '" AND time_in <= "' . $toTime . '"');
                $objDatatables->or_where('time_date = "' . $fromDate . '" AND time_out >= "' . $fromTime . '"');
            } else {
                $objDatatables->where('time_date <=', $toDate);
                $objDatatables->where('time_out <=', $toTime);
            }
        }
    ...
    public function export_time_schedule($isFromMain = FALSE) {

        if ($isFromMain) {

            $this->db->select("time_schedule_id, sma_users.id, sma_users.first_name, sma_users.last_name, time_date, MIN(time_in) AS time_in, MAX(time_out) AS time_out, TIME(SUM(TIMEDIFF(time_out, time_in))) AS total_time, sma_time_schedule.ip_address", FALSE)
                    ->from("sma_time_schedule")
                    ->join('sma_users', 'sma_users.id = sma_time_schedule.user_id')
                    ->group_by('time_date, sma_users.id')
                    ->order_by('time_date', 'desc');

            if ($this->input->get('user') && $this->input->get('user') != '') {
                $this->db->where('sma_users.id', $this->input->get('user'));
            }

            if ($this->input->get('from_date') && $this->input->get('from_date') != '') {
                $fromDateTime = explode(' ', $this->input->get('from_date'));
                $fromDate = DateTime::createFromFormat('d/m/Y', $fromDateTime[0])->format('Y-m-d');
                $fromTime = $fromDateTime[1];
                if ($this->input->get('to_date') && $this->input->get('to_date') != '') {
                    $toDateTime = explode(' ', $this->input->get('to_date'));
                    $toDate = DateTime::createFromFormat('d/m/Y', $toDateTime[0])->format('Y-m-d');
                    $toTime = $toDateTime[1];
                    $this->db->where('time_date > "' . $fromDate . '" AND time_date < "' . $toDate . '"');
                    $this->db->or_where('time_date = "' . $fromDate . '" AND time_in >= "' . $fromTime . '"');
                    $this->db->or_where('time_date = "' . $toDate . '" AND time_out <= "' . $toTime . '"');
                } else {
                    $this->db->where('time_date >=', $fromDate);
                    $this->db->where('time_in >=', $fromTime);
                }
            }
            ...
            if ($this->input->get('to_date') && $this->input->get('to_date') != '') {
                ...
                if ($this->input->get('from_date') && $this->input->get('from_date') != '') {
                    ...
                    $fromDate = DateTime::createFromFormat('d/m/Y', $fromDateTime[0])->format('Y-m-d');
                    $fromTime = $fromDateTime[1];
                    $this->db->where('time_date < "' . $toDate . '" AND time_date > "' . $fromDate . '"');
                    $this->db->or_where('time_date = "' . $toDate . '" AND time_in <= "' . $toTime . '"');
                    $this->db->or_where('time_date = "' . $fromDate . '" AND time_out >= "' . $fromTime . '"');
                } else {
                    $this->db->where('time_date <=', $toDate);
                    $this->db->where('time_out <=', $toTime);
                }
            }

            $arrResult = $this->db->get()->result();
        } else {
            $strFromDate = $this->input->get('from');
            $strToDate = $this->input->get('to');
            $intUser = $this->input->get('user');
            $this->db->select("sma_users.id, sma_users.first_name, sma_users.last_name, time_date, MIN(time_in) AS time_in, MAX(time_out) AS time_out, TIME(SUM(TIMEDIFF(time_out, time_in))) AS total_time, sma_time_schedule.ip_address", FALSE)
                    ->join('sma_users', 'sma_users.id = sma_time_schedule.user_id')
                    ->where('time_date >=', $strFromDate)
                    ->where('time_date <=', $strToDate)
                    ->group_by('time_date, sma_users.id')
                    ->order_by('time_date', 'desc');

            if ($intUser) {
                $this->db->where('user_id', $intUser);
            }

            $arrResult = $this->db->get("sma_time_schedule")->result();
        }
        ...
        $this->load->library('excel');
        $this->excel->setActiveSheetIndex(0);
        $this->excel->getActiveSheet()->setTitle('time_schedule_' . date('Y-m-d'));
        $this->excel->getActiveSheet()->SetCellValue('A1', lang('first_name'));
        $this->excel->getActiveSheet()->SetCellValue('B1', lang('last_name'));
        $this->excel->getActiveSheet()->SetCellValue('C1', lang('Time In'));
        $this->excel->getActiveSheet()->SetCellValue('D1', lang('Time Out'));
        $this->excel->getActiveSheet()->SetCellValue('E1', lang('Date'));
        $this->excel->getActiveSheet()->SetCellValue('F1', lang('Hours'));
        $this->excel->getActiveSheet()->SetCellValue('G1', lang('IP Address'));
        ...
        $row = 2;
        $totalTime = '00:00:00';
        foreach ($arrResult as $key => $report) {

            $emp_id = $report->id; //employee id

            if (!$isFromMain) {
                if ($flag != "{$emp_id}_{$strFromDate}_{$strToDate}" && $flag != '') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, "{$h}H : {$m}M");
            ...
            $this->excel->getActiveSheet()->SetCellValue('A' . $row, $report->first_name);
            $this->excel->getActiveSheet()->SetCellValue('B' . $row, $report->last_name);
            $this->excel->getActiveSheet()->SetCellValue('C' . $row, $report->time_in);
            $this->excel->getActiveSheet()->SetCellValue('D' . $row, $report->time_out);
            $this->excel->getActiveSheet()->SetCellValue('E' . $row, $report->time_date);
            $this->excel->getActiveSheet()->SetCellValue('F' . $row, $report->total_time);
            $this->excel->getActiveSheet()->SetCellValue('G' . $row, $report->ip_address);
            $row++;

            if (!$isFromMain) {
                $flag = "{$emp_id}_{$strFromDate}_{$strToDate}";
            }

            $totalTime = sum_the_time($report->total_time, $totalTime);
        }

        //add total at the end of record
        if ($arrResult) {
            list($h, $m, $s) = explode(':', $totalTime);
        ...
        $filename = 'time_schedule_' . date('Y_m_d_H_i_s');

        if ($this->input->get('pdf') || $isFromMain) {
    ...
    function product_details() {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('product_details_report')));
        $meta = array('page_title' => lang('product_details_report'), 'bc' => $bc);
        $this->page_construct('reports/product_details', $meta, $this->data);
    }


    function getProductDetailsReport($pdf = NULL, $xls = NULL) {

        $this->sma->checkPermissions('product_details');
        ...
        if ($pdf || $xls) {
            ...
            if($warehouse){
                $this->db
                ->select($this->db->dbprefix('products') . ".code, " . $this->db->dbprefix('products') . ".name,
				{$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname,{$this->db->dbprefix('units')}.name as uname," . $this->db->dbprefix('products') . ".cost," . $this->db->dbprefix('products') . ".price,{$this->db->dbprefix('warehouses_products')}.quantity as qty,{$this->db->dbprefix('products')}.alert_quantity,
                {$this->db->dbprefix('products')}.id as id", FALSE)
                    ...
                    ->where('warehouses_products.warehouse_id', $warehouse)
                ...
                $this->db
                    ->select($this->db->dbprefix('products') . ".code, " . $this->db->dbprefix('products') . ".name,
				{$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname,{$this->db->dbprefix('units')}.name as uname," . $this->db->dbprefix('products') . ".cost," . $this->db->dbprefix('products') . ".price,{$this->db->dbprefix('products')}.quantity as qty,{$this->db->dbprefix('products')}.alert_quantity,
                {$this->db->dbprefix('products')}.id as id", FALSE)
                    ->from('products')
                    ->join('categories', 'products.category_id=categories.id', 'left')
                    ->join('units', 'products.unit=units.id', 'left')
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('E1', lang('um'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->uname);
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->cost);
                    $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->price);
                    $this->excel->getActiveSheet()->SetCellValue('H' . $row, $data_row->qty);
                    $this->excel->getActiveSheet()->SetCellValue('I' . $row, $data_row->alert_quantity);
                    $row++;
                }
            ...
            echo $this->datatables->generate();
             ...
             if ($brand) {
                 $this->datatables->where($this->db->dbprefix('products') . ".warehouse", $warehouse);
             }
