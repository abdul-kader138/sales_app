...
$('#podiscount').focus(function () {
    ...
    /* --------------------------
     * Edit Row Min selling price Method 
     -------------------------- */
     var old_row_qty;
     $(document).on("focus", '.min_selling_price', function () {
     old_row_qty = $(this).val();
    }).on("change", '.min_selling_price', function () {
        var row = $(this).closest('tr');
        if (!is_numeric($(this).val())) {
            $(this).val(old_row_qty);
            bootbox.alert(lang.unexpected_value);
        ...
        var new_selling_price = parseFloat($(this).val()),
        item_id = row.attr('data-item-id');
        if (new_selling_price < poitems[item_id].row.real_unit_cost) {
			 $(this).val(old_row_qty);
            bootbox.alert("invalid amount");
            return;
		}
			
		poitems[item_id].row.min_selling_price = new_selling_price;
        localStorage.setItem('poitems', JSON.stringify(poitems));
        loadItems();
    });
    ...
    /* --------------------------
     * Edit Row Landing price Method 
    -------------------------- */
     var old_row_qty;
     $(document).on("focus", '.landing_price', function () {
     old_row_qty = $(this).val();
    }).on("change", '.landing_price', function () {
        var row = $(this).closest('tr');
        if (!is_numeric($(this).val())) {
            $(this).val(old_row_qty);
            bootbox.alert(lang.unexpected_value);
        ...
        var new_landing_price = parseFloat($(this).val()),
        item_id = row.attr('data-item-id');
		if (new_landing_price < poitems[item_id].row.real_unit_cost) {
			 $(this).val(old_row_qty);
            bootbox.alert("invalid amount");
            return;
		}
        poitems[item_id].row.landing_price = new_landing_price;
        localStorage.setItem('poitems', JSON.stringify(poitems));
        loadItems();
    });
