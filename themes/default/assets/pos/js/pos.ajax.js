...
$('#posdiscount').focus(function () {
    ...
    $(document).on('click', '.edit', function () {
        ...
        discount = row.children().children('.rdiscount').val();
         w_qty = formatDecimal(row.children().children('.w_qty').val());
         var cost_price = formatDecimal(row.children().children('.product_cost').val(),2);
        ...
        var min_selling_price = item.row.min_selling_price;
        var landing_price = item.row.landing_price;
        ...
        $('#leddingprice').val(formatDecimal(landing_price));
        $('#pminselling').val(formatDecimal(min_selling_price));
    ...
    $(document).on('click', '#editItem', function () {
        ...
        var pminselling = parseFloat($('#pminselling').val());
        var leddingprice = parseFloat($('#leddingprice').val());
        
        if(price < pminselling || price < leddingprice){
            document.getElementById('selling_price_message').style.display = "block";
            return false;
        }else{document.getElementById('selling_price_message').style.display = "none";}
...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var product_id = item.row.id,item_cost=item.row.cost,w_qty = item.w_qty, item_type = item.row.type, combo_items = item.combo_items, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            ...
            var msp = ( item.row.min_selling_price >= item.row.landing_price ? item.row.min_selling_price : item.row.landing_price );
            ...
            // tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + ( formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val)) != '0.00' ? formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val)) : formatMoney(parseFloat(msp) + parseFloat(pr_tax_val)) != '0.00' ? formatMoney(parseFloat(msp) + parseFloat(pr_tax_val)) : formatMoney(parseFloat(item.row.cost) + parseFloat(pr_tax_val)) ) + '</span></td>';
            tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(parseFloat(item_price) ) + '</span></td>';
            tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="tax_' + row_no + '">' + formatMoney( parseFloat(pr_tax_val)) + '</span></td>';
            ...
            tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney((( parseFloat(item_price) + parseFloat(pr_tax_val) != '0.00' ? parseFloat(item_price) + parseFloat(pr_tax_val) : parseFloat(msp) + parseFloat(pr_tax_val) != '0.00' ? parseFloat(msp) + parseFloat(pr_tax_val) : parseFloat(item.row.cost) + parseFloat(pr_tax_val) ) * parseFloat(item_qty))) + '</span></td>';
            ...
            total += formatDecimal((( parseFloat(item_price) + parseFloat(pr_tax_val) != '0.00' ? parseFloat(item_price) + parseFloat(pr_tax_val) : parseFloat(msp) + parseFloat(pr_tax_val) != '0.00' ? parseFloat(msp) + parseFloat(pr_tax_val) : parseFloat(item.row.cost) + parseFloat(pr_tax_val) ) * parseFloat(item_qty)), 4);
