                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group all">
                        ...
                        <div class="input-group">
                            <?= form_input('code', $code_number, 'class="form-control" id="code"  required="required"') ?>
                   ...
                   <div class="form-group all">
                        ...
                        <?= form_input('price', (isset($_POST['price']) ? $_POST['price'] : ($product ? $this->sma->formatDecimal($product->price) : '')), 'class="form-control tip" id="price"') ?>
                  </div>
                    ...
                    <?php if ($Settings->tax1) { ?>
                        ...
                        <div class="form-group all">
                            ...
                            echo form_dropdown('tax_rate', $tr, (isset($_POST['tax_rate']) ? $_POST['tax_rate'] : '5'), 'class="form-control select" id="tax_rate" placeholder="' . lang("select") . ' ' . lang("product_tax") . '" style="width:100%"')
...
<?php } ?>
    $(document).ready(function () {
        ...
        //Unit Fild Default Selected This Code Exicuted or Not
        var unitId = $("#unit").val();
        if(unitId){
            $.ajax({
                type: "get",
                async: false,
                url: "<?= site_url('products/getSubUnits') ?>/" + unitId,
                dataType: "json",
                success: function (data) {
                    ...
                    $.each(data, function () {
                        $("<option />", {value: this.id, text: this.name}).appendTo($('#default_sale_unit'));
                        $("<option />", {value: this.id, text: this.name}).appendTo($('#default_purchase_unit'));
                    });
                    $('#default_sale_unit').select2('val', unitId);
                    $('#default_purchase_unit').select2('val', unitId);
                },
                error: function () {
                    bootbox.alert('<?= lang('ajax_error') ?>');
                }
        ...
        //Unit Fild selected Change this code Exicuted or Not
