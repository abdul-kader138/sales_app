...
<header id="header" class="navbar">
    <div class="container">
        <a class="navbar-brand" href="<?= site_url() ?>"><span class="logo"><span
            ...
            <ul class="nav navbar-nav pull-right">
                <li class="dropdown">
                    <a class="btn account dropdown-toggle" data-toggle="dropdown" href="#">
                        <img alt=""
                             src="<?= $this->session->userdata('avatar') ? site_url() . 'assets/uploads/avatars/thumbs/' . $this->session->userdata('avatar') : $assets . 'images/' . $this->session->userdata('gender') . '.png'; ?>"
            ...
            <ul class="nav navbar-nav pull-right">
                ...
                <li class="dropdown">
                    <a class="btn pos-tip" title="<?= lang('view_bill_screen') ?>" data-placement="bottom"
                       href="<?= site_url('pos/view_bill') ?>" target="_blank">
                ...
                <li class="dropdown">
                    <a class="btn blightOrange pos-tip" id="opened_bills"
                       ...
                       href="<?= site_url('pos/opened_bills') ?>" data-toggle="ajax">
                        <i class="fa fa-th"></i>
                    </a>
                </li>
                <li class="dropdown">
                    <a class="btn bdarkGreen pos-tip" id="register_details"
                       ...
                       href="<?= site_url('pos/register_details') ?>" data-toggle="modal" data-target="#myModal">
                ...
                <li class="dropdown">
                    <a class="btn borange pos-tip" id="close_register"
                       ...
                       data-backdrop="static" href="<?= site_url('pos/close_register') ?>" data-toggle="modal"
                       data-target="#myModal">
                ...
                <?php if ($Owner) { ?>
                    <li class="dropdown">
                        <a class="btn bdarkGreen pos-tip" id="today_profit"
                           ...
                           href="<?= site_url('reports/profit') ?>" data-toggle="modal" data-target="#myModal">
                            <i class="fa fa-hourglass-half"></i>
                        </a>
                    </li>
                <?php } ?>
                <li class="dropdown">
                    <a class="btn bdarkGreen pos-tip" id="today_sale" title="<span><?= lang('today_sale') ?></span>"
                ...
                <?php if ($Owner || $Admin) { ?>
                    <li class="dropdown hidden-xs">
                        <a class="btn bblue pos-tip" title="<?= lang('list_open_registers') ?>" data-placement="bottom"
        ...
        <div class="input-group">
            ...
            <?php if ($Owner || $Admin || $GP['customers-add']) { ?>
                <div class="input-group-addon no-print" style="padding: 2px 8px;">
                    <a href="<?= site_url('customers/add'); ?>" id="add-customer" class="external" data-toggle="modal"
                       data-target="#myModal">
...
<div id="print">
    ...
    <div id="left-bottom">
        ...
        <div id="payment-con">
                <input type="hidden" name="cc_charges_1" id="cc_charges_1?>" value=""/>
            <?php for ($i = 1; $i <= 5; $i++) { ?>
        ...
        <input name="order_tax" type="hidden"
               value="<?= $suspend_sale ? $suspend_sale->order_tax_id : $Settings->default_tax_rate2; ?>" id="postax2">
        <input name="discount" type="hidden" value="<?= $suspend_sale ? $suspend_sale->order_discount_id : ''; ?>"
               id="posdiscount">
        <input name="shipping" type="hidden" value="<?= $suspend_sale ? $suspend_sale->shipping : '0'; ?>"
               id="posshipping">
                ...
                <div id="ajaxproducts">
                    ...
                    <div class="btn-group btn-group-justified pos-grid-nav">
                        ...
                        <?php if ($Owner || $Admin || $GP['sales-add_gift_card']) { ?>
                            <div class="btn-group">
                                <button style="z-index:10003;" class="btn btn-primary pos-tip" type="button"
                                        id="sellGiftCard" title="<?= lang('sell_gift_card') ?>">
                        ...
                        ?>
                        <div class="btn-group">
                            <button style="z-index:10004;" class="btn btn-primary pos-tip" title="<?= lang('next') ?>"
                                    type="button" id="next">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" id="pr_popover_content">
                <form class="form-horizontal" role="form">
                    ...
                    <?php } ?>
                    <div class="form-group">
                        <label for="pquantity" class="col-sm-4 control-label"><?= lang('stock_quantity') ?></label>

                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="pwquantity" readonly="readonly">
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <input type="text" class="form-control kb-pad" id="leddingprice" readonly>
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="pminselling" class="col-sm-4 control-label"><?= lang('Min Selling Price') ?></label>

                        <div class="col-sm-8">
                            <input type="text" class="form-control kb-pad" id="pminselling" disabled>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="pminselling" class="col-sm-4 control-label"><?= lang('cost_unit_price') ?></label>

                        <div class="col-sm-8">
                            <input type="text" class="form-control kb-pad" id="costPrice" disabled>
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="pprice" class="col-sm-4 control-label"><?= lang('sale_unit_price') ?></label>
                    ...
                    <table class="table table-bordered table-striped">
                        <tr>
                            <th style="width:25%;"><?= lang('sale_unit_price'); ?></th>
...
$(document).ready(function () {
    ...
    function calculateTotals() {
        ...
        var total_paying = 0;
        var ia = $(".amount");
        var card_charge = $("#cc_charge_1").val();
        $.each(ia, function (i) {
        ...
        <?php if ($pos_settings->rounding) {?>
        if (card_charge && !isNaN(card_charge)) {
            $('#balance').text(formatMoney(total_paying - (round_total+ parseFloat((total_amount * card_charge) / 100))));
            $('#balance_' + pi).val(formatDecimal(total_paying - (round_total+ parseFloat((total_amount * card_charge) / 100))));
        } else {
        ...
        <?php } else {?>
        if (card_charge && !isNaN(card_charge)) {
            $('#balance').text(formatMoney(total_paying - (gtotal+ parseFloat((total_amount * card_charge) / 100))));
            $('#balance_' + pi).val(formatDecimal(total_paying - (gtotal + parseFloat((total_amount * card_charge) / 100))));
        }
        else
        {
